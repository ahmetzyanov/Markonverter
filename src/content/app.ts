...
function isOzonProductSilentSwept(productId: string): boolean {
  try {
...
function shouldStartOzonSilentPriceSweep(
  product: ProductIdentity,
  settings: ExtensionSettings,
  comparisonPoints: PickupPoint[]
): boolean {
...
async function runOzonSilentPriceSweep(
  product: ProductIdentity,
  settings: ExtensionSettings,
  comparisonPoints: PickupPoint[],
  runId: number
...
async function waitForOzonDeliveryContainerToClose(timeoutMs: number): Promise<void> {
...
function extractOzonPickupCandidateFromRow(element: HTMLElement): OzonPickupCandidate | null {
  ...
  const matched = matchDetectedPickupCandidateToRow(text, latestPickupCandidates);
  if (!matched) {
    return null;
  }
  return {
...
async function saveDetectedPickupCandidate(candidate: OzonPickupCandidate, product: ProductIdentity): Promise<void> {
  ...
  if (!response.ok || !("settings" in response)) {
    captureStatus = { tone: "error", message: pickupSaveErrorMessage(response) };
    renderLastPanel();
